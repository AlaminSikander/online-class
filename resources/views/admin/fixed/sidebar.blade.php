<div class="sidebar" id="sidebar">
    <div class="sidebar-inner slimscroll">
        <div id="sidebar-menu" class="sidebar-menu">
            <ul>
                <li class="menu-title">
                    <span>Main Menu</span>
                </li>
                <li class="">
                    <a href="#"><i class="fas fa-user-graduate"></i> <span> Dashboard</span></a>
                </li>
                <li class=""> 
                    <a href="{{route('admin.student')}}"><i class="fas fa-user-graduate"></i> <span> Students</span></a>
                </li>
                <li class="">
                    <a href="{{route('admin.teacher')}}"><i class="fas fa-chalkboard-teacher"></i> <span> Teachers</span></a>
                </li>
                <li class="">
                    <a href="{{route('admin.subject')}}"><i class="fas fa-book-reader"></i> <span>Subjects</span></a>
                </li>
                <li class="">
                    <a href="{{route('admin.exam')}}"><i class="fas fa-clipboard-list"></i> <span>Exam Schedule</span></a>
                </li>
                <li class="">
                    <a href="{{route('admin.classSchedule')}}"><i class="fas fa-table"></i> <span>Class Schedule</span></a>
                </li>
            </ul>
        </div>
    </div>
</div>
